...
pub const PARSE_L4: &str =
    r#"SELECT a, sum(b) FROM t WHERE a > 10 AND c LIKE "%foo%" GROUP BY a HAVING sum(b) > 100 ORDER BY a ASC"#;
...
pub const PARSE_L6: &str =
    "SELECT a, b FROM t WHERE (a IN (10, 20, 30)) AND b IS NOT NULL UNION SELECT c, d FROM t2 ORDER BY a ASC LIMIT 50";
...
pub const EXEC_E3: &str =
    r#"SELECT elbname, elb_status_code FROM elb WHERE elb_status_code = "200" ORDER BY elbname ASC"#;
pub const EXEC_E4: &str =
...
pub const PROF_SCAN_FILTER_GROUPBY: &str =
    r#"SELECT elb_status_code, count(*) FROM elb WHERE elb_status_code = "200" GROUP BY elb_status_code"#;
