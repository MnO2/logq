...
- **WS3:** Migrated from clap 2's YAML configuration to clap 4's derive API, with version metadata sourced directly from `Cargo.toml`. Replaced the unmaintained `json` crate with `serde_json`, preserving JSONL field order and making nested objects/arrays round-trip through JSON output.
