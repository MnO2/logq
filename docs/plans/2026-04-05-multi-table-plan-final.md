# Plan: Multi-Table CLI Support (Final)
...
| **C3** | Enumerated all 26 tests in `app.rs` that directly construct `DataSource` and pass it to `run()`/`run_to_vec()`. Split Step 5 into 5A (function signatures), 5B (test helpers), 5C (individual test updates). |
...
There are 26 test functions that directly construct `DataSource` and pass it to `run()`/`run_to_vec()`. Each requires wrapping the `DataSource` in a one-entry `DataSourceRegistry`.
...
### 8f. Test for duplicate table name error

```rust
#[test]
fn test_duplicate_table_name_errors() {
    let dir = tempdir().unwrap();
    let file_a = dir.path().join("a.jsonl");
    let mut fa = File::create(file_a.clone()).unwrap();
    writeln!(fa, r#"{{"x": 1}}"#).unwrap();
    fa.sync_all().unwrap();
    drop(fa);

    let file_b = dir.path().join("b.jsonl");
    let mut fb = File::create(file_b.clone()).unwrap();
    writeln!(fb, r#"{{"x": 2}}"#).unwrap();
    fb.sync_all().unwrap();
    drop(fb);

    // Duplicate table name "a" — should be caught at CLI parse time,
    // but test at registry level: inserting same key twice
    let mut data_sources = common::types::DataSourceRegistry::new();
    data_sources.insert(
        "a".to_string(),
        common::types::DataSource::File(file_a, "jsonl".to_string(), "a".to_string()),
    );
    // Second insert with same key overwrites — CLI layer prevents this,
    // but query should still work with single entry
    let results = run_to_vec(r#"select x from a"#, data_sources);
...
#[test]
fn test_table_name_case_mismatch_errors() {
    let dir = tempdir().unwrap();
    let file_a = dir.path().join("a.jsonl");
    let mut fa = File::create(file_a.clone()).unwrap();
    writeln!(fa, r#"{{"x": 1}}"#).unwrap();
    ...
    data_sources.insert(
        "MyTable".to_string(),
        common::types::DataSource::File(file_a, "jsonl".to_string(), "MyTable".to_string()),
    );

    // Case mismatch: "mytable" vs "MyTable"
    let result = run_to_vec(r#"select x from mytable"#, data_sources);
    assert!(result.is_err());
    let err_msg = format!("{}", result.unwrap_err());
    assert!(err_msg.contains("Unknown table"));
    assert!(err_msg.contains("MyTable"));
...
| 8 | Step 8 | No (depends on 5C) | `app.rs` |
