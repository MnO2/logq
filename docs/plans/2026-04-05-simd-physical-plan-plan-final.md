# Plan: SIMD-Accelerated Physical Plan for logq (Final)
...
|-------|----------|---------|------------|
| **C1** | Critical | `typed_column_get_value` for DateTime loses timezone (always reconstructs as UTC); HttpRequest/Host values in non-Mixed columns undocumented | Documented that DateTime roundtrip converts to UTC, which is acceptable for log queries where timestamps are typically UTC. Added a code comment in `typed_column_get_value` explaining the tradeoff. Documented that HttpRequest/Host values in non-Mixed columns are treated as NULL (type mismatch) -- this should not happen in practice since the schema maps these types to Mixed. |
| **C2** | Critical | `Bitmap::or()` silently truncates when bitmaps have different word counts because `zip()` stops at the shorter iterator; `and()` has a similar but less dangerous variant | Fixed `Bitmap::or()` to iterate `max(words)`, using `.get(i).unwrap_or(&0)` for the shorter bitmap. Fixed `Bitmap::and()` to iterate `min(words)` explicitly. Added `test_bitmap_or_different_lengths` and `test_bitmap_and_different_lengths` tests. |
| **C3** | Critical | `AHasher::default()` creates a new hasher per element, which is expensive (initialization involves multiple state words) and not the recommended stable API | Replaced with `ahash::RandomState::with_seeds(fixed_seeds)` + `hash_one()` for deterministic, efficient hashing. A single `RandomState` is created per column, and `hash_one()` is called per string. |
| **C4** | Critical | Step 14 (JSONL simd-json migration) was too vague -- no implementation code, no `simd_json_value_to_data_model` function, no buffer handling, no integration test | Provided complete implementation: `simd_json_value_to_data_model` function, modified JSONL path in `read_record` with `Vec<u8>` buffer handling, `ReaderError::ParseSimdJson` variant, and an integration test that parses a JSONL line through the actual `Reader`. |
...
mod tests {
    ...
    #[test]
    fn test_bitmap_or_different_lengths() {
        // a has 64 bits (1 word), b has 128 bits (2 words)
        let mut a = Bitmap::all_unset(64);
        a.set(0);
        let mut b = Bitmap::all_unset(128);
        b.set(0);
        b.set(100); // in word 1, which a doesn't have

        let or = a.or(&b);
        assert_eq!(or.len, 128); // max of the two lengths
        assert!(or.is_set(0));   // both had it
        assert!(or.is_set(100)); // only b had it, must not be truncated
        assert_eq!(or.count_ones(), 2);
    }

    #[test]
    fn test_bitmap_and_different_lengths() {
        // a has 128 bits (2 words), b has 64 bits (1 word)
        let mut a = Bitmap::all_set(128);
        let mut b = Bitmap::all_set(64);

        let and = a.and(&b);
        assert_eq!(and.len, 64); // min of the two lengths
        // Only the first 64 bits should be present
        assert_eq!(and.count_ones(), 64);
    }
...
impl Bitmap {
    ...
    /// Bitwise AND of two bitmaps. Result length is `min(self.len, other.len)`.
    /// Only the words covered by both bitmaps are included; extra words from
    /// the longer bitmap are dropped (AND with implicit zeros).
    ...
    /// Bitwise OR of two bitmaps. Result length is `max(self.len, other.len)`.
    /// When one bitmap is shorter, missing words are treated as 0 (unset),
    /// so bits from the longer bitmap are preserved in the result.
...
///
/// Uses `ahash::RandomState` with fixed seeds for deterministic hashing.
/// A single `RandomState` is created per call and `hash_one()` is invoked
/// per string, avoiding the overhead of constructing a new `AHasher` per element.
pub fn hash_column_utf8(data: &[u8], offsets: &[u32], hashes: &mut [u64]) {
    // Fixed seeds ensure deterministic hashing across runs.
    // This is acceptable for non-adversarial use (log query engine).
    let state = ahash::RandomState::with_seeds(
        0x1234_5678_9abc_def0,
        0xfedd_ba98_7654_3210,
...
///
/// Design notes on lossy conversions:
///
/// - **DateTime**: Converts epoch microseconds back to `DateTime<FixedOffset>` using UTC
///   (offset=0). The original timezone offset is lost. This is acceptable for log queries
///   where timestamps are typically UTC (AWS ELB/ALB/S3 logs always use UTC). If non-UTC
///   timezone preservation becomes necessary, the DateTime variant would need to store
///   a parallel `Vec<i32>` of UTC offset seconds alongside the epoch micros.
///
/// - **HttpRequest/Host**: These Value variants should never appear in non-Mixed columns.
///   The schema inference always maps Host and HttpRequest fields to ColumnType::Mixed
///   (see design document Section 1). If a Value::HttpRequest or Value::Host were
///   pushed into a non-Mixed ColumnBuilder (e.g., ColumnType::Utf8), it would be treated
///   as a type mismatch and stored as NULL. In practice this cannot happen because:
///   (a) structured log schemas explicitly map Host/HttpRequest fields to Mixed, and
///   (b) JSONL schema inference never produces Host/HttpRequest values (those are only
///   created by the structured log tokenizer).
fn typed_column_get_value(col: &TypedColumn, idx: usize) -> Value {
    match col {
        ...
        TypedColumn::DateTime { data, null, missing } => {
            if !missing.is_set(idx) { return Value::Missing; }
            if !null.is_set(idx) { return Value::Null; }
            // NOTE: Timezone information is lost -- always reconstructs as UTC.
            // This is acceptable for log queries where timestamps are typically UTC
            // (all AWS log formats use UTC). The epoch microseconds are timezone-neutral,
            // so the actual instant in time is preserved; only the display offset changes.
        ...
        let typed_value = match value {
            ...
            other => {
                ...
                if matched {
                    ...
                    // This handles cases like Value::HttpRequest or Value::Host
                    // being pushed into a non-Mixed column. In practice this
                    // should not happen because the schema maps Host/HttpRequest
                    // fields to ColumnType::Mixed.
                    self.missing.push(true);
                    self.null.push(false);
                    None
                }
...
This step changes the JSONL parsing path to use `simd-json` with mutable `Vec<u8>` buffers, as specified in Phase 1 of the design document. The current code uses the `json` crate's `json::parse(&self.buf)` which borrows immutably; `simd-json` requires mutable ownership of the buffer because it modifies it in-place for zero-copy parsing.

### 14a. Write failing test

Add to the existing `tests` module in `src/execution/datasource.rs`:

```rust
#[test]
fn test_simd_json_value_to_data_model_object() {
    // Verify the simd_json -> Value conversion matches json crate behavior
    let mut buf = br#"{"name":"alice","age":30,"active":true,"score":3.14}"#.to_vec();
    let owned: simd_json::OwnedValue = simd_json::to_owned_value(&mut buf).unwrap();
    let result = simd_json_value_to_data_model(&owned);

    match result {
        Value::Object(o) => {
            assert_eq!(o.get("name"), Some(&Value::String("alice".to_string())));
            assert_eq!(o.get("age"), Some(&Value::Int(30)));
            assert_eq!(o.get("active"), Some(&Value::Boolean(true)));
            // Float: 3.14 as f32
            match o.get("score") {
                Some(Value::Float(_)) => {} // correct type
...
#[test]
fn test_simd_json_value_to_data_model_nested() {
    let mut buf = br#"{"a":{"b":1},"c":[1,2,3]}"#.to_vec();
    let owned: simd_json::OwnedValue = simd_json::to_owned_value(&mut buf).unwrap();
    let result = simd_json_value_to_data_model(&owned);
...
#[test]
fn test_simd_json_value_to_data_model_null() {
    let mut buf = br#"{"x":null}"#.to_vec();
    let owned: simd_json::OwnedValue = simd_json::to_owned_value(&mut buf).unwrap();
    let result = simd_json_value_to_data_model(&owned);

    match result {
        Value::Object(o) => {
            assert_eq!(o.get("x"), Some(&Value::Null));
        }
        _ => panic!("expected Object"),
    }
}
...
#[test]
fn test_jsonl_reader_with_simd_json() {
    // Integration test: parse a JSONL line through the actual Reader
    let content = r#"{"name":"alice","age":30,"active":true}
{"name":"bob","age":25,"active":false}
"#;
    let mut reader = ReaderBuilder::new("jsonl".to_string())
        .with_reader(std::io::BufReader::new(content.as_bytes()));

    // First record
    let record = reader.read_record().unwrap().unwrap();
    let vars = record.into_variables();
    assert_eq!(vars.get("name"), Some(&Value::String("alice".to_string())));
    assert_eq!(vars.get("age"), Some(&Value::Int(30)));
    assert_eq!(vars.get("active"), Some(&Value::Boolean(true)));

    // Second record
    let record = reader.read_record().unwrap().unwrap();
    let vars = record.into_variables();
    assert_eq!(vars.get("name"), Some(&Value::String("bob".to_string())));
    assert_eq!(vars.get("age"), Some(&Value::Int(25)));
    assert_eq!(vars.get("active"), Some(&Value::Boolean(false)));
...
```bash
cd /Users/paulmeng/Develop/logq && cargo test --lib execution::datasource::tests::test_simd_json
cd /Users/paulmeng/Develop/logq && cargo test --lib execution::datasource::tests::test_jsonl_reader_with_simd_json
```

### 14c. Implementation

**Step 14c-1: Add `simd_json_value_to_data_model` function**

Add this function near the existing `json_to_data_model` in `src/execution/datasource.rs`:

```rust
/// Convert a simd_json OwnedValue to the logq data model Value.
/// This is the simd-json equivalent of `json_to_data_model`.
///
/// Number handling mirrors the existing `json_to_data_model` behavior:
/// integers that fit in i32 become Value::Int, others become Value::Float.
fn simd_json_value_to_data_model(val: &simd_json::OwnedValue) -> Value {
    use simd_json::OwnedValue;
    match val {
        OwnedValue::Object(o) => {
            let t: LinkedHashMap<String, Value> = o
                .iter()
                .map(|(k, v)| (k.to_string(), simd_json_value_to_data_model(v)))
                .collect();
            Value::Object(Box::new(t))
        }
        OwnedValue::Array(a) => {
            let a: Vec<Value> = a.iter().map(|v| simd_json_value_to_data_model(v)).collect();
            Value::Array(a)
        }
        OwnedValue::Null => Value::Null,
        OwnedValue::Static(simd_json::StaticNode::Bool(b)) => Value::Boolean(*b),
        OwnedValue::Static(simd_json::StaticNode::Null) => Value::Null,
        OwnedValue::Static(simd_json::StaticNode::I64(n)) => {
            // Match existing behavior: use i32 if it fits, otherwise f32
            if *n >= i32::MIN as i64 && *n <= i32::MAX as i64 {
                Value::Int(*n as i32)
            } else {
                Value::Float(OrderedFloat::from(*n as f32))
            }
        }
        ...
        OwnedValue::Static(simd_json::StaticNode::F64(f)) => {
            // Check if this is actually an integer value (e.g., 30.0 -> Int(30))
            // to match the existing json crate behavior where numbers without
            // a fractional part become Int.
            if f.fract() == 0.0 && *f >= i32::MIN as f64 && *f <= i32::MAX as f64 {
                Value::Int(*f as i32)
            } else {
                Value::Float(OrderedFloat::from(*f as f32))
            }
        }
...
**Step 14c-2: Add `ParseSimdJson` to `ReaderError`**

Add a new variant to the `ReaderError` enum in `src/execution/datasource.rs`:
...
pub enum ReaderError {
    ...
    #[error("{0}")]
    ParseSimdJson(#[from] simd_json::Error),
}
```

**Step 14c-3: Modify the JSONL path in `read_record`**

Replace the existing JSONL branch in the `read_record` method (lines 935-951 of `src/execution/datasource.rs`):
...
```rust
    ...
    // simd-json requires a mutable buffer. Take ownership of self.buf's
    // contents as Vec<u8>, parse, then restore self.buf for reuse.
    // The trim_end() removes the trailing newline before parsing.
    let trimmed = self.buf.trim_end();
    let mut bytes = trimmed.as_bytes().to_vec();
    let owned = simd_json::to_owned_value(&mut bytes)?;
    let data_model = simd_json_value_to_data_model(&owned);
...
Note: We use `trimmed.as_bytes().to_vec()` rather than `self.buf.into_bytes()` because `self.buf` is reused across calls (cleared at the top of `read_record` via `self.buf.clear()`). The allocation of the `Vec<u8>` copy is acceptable because `simd-json`'s parsing speedup far exceeds the cost of this copy. In a future optimization, the `Reader` struct could be changed to use a `Vec<u8>` buffer directly (replacing `String`) to avoid the copy, but that is a larger refactor.

**Step 14c-4: Add `simd_json` import**

Add to the imports at the top of `src/execution/datasource.rs`:
