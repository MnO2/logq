# Plan: PrefixSort Optimization for ORDER BY
...
/// NaN -> 0xFFFFFFFF, +Inf -> 0xFFFFFFFE, -Inf -> 0x00000000.
...
```bash
cd /Users/paulmeng/Develop/logq && cargo test --lib test_elb_order_by_timestamp test_elb_limit_with_order test_alb_order_by_received_bytes test_integration_distinct_and_order_by
```
...
criterion_group!(benches, bench_sort_int, bench_sort_string, bench_sort_multi_key);
...
```bash
cd /Users/paulmeng/Develop/logq && cargo test test_elb_order_by_timestamp test_elb_limit_with_order test_alb_order_by_received_bytes test_integration_distinct_and_order_by
```
