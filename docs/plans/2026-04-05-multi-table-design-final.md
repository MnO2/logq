# Multi-Table CLI Support Design (Final)

## Changes from v2

| Issue | What changed |
|-------|-------------|
| **Review 2 C1** | Removed dead `is_empty()` check in `check_env_ref` — it runs after `path_segments[0]` access so can never trigger. Reordered to check emptiness first. |
| **Review 2 C2** | `SELECT *` with `GROUP BY` in multi-table queries errors: no single schema to expand into. Added `StarGroupByMultiTable` error variant. |
| **Review 2 S5** | Sort registry keys in `UnknownTable` error message for deterministic output. |

## Changes from v1

This revision addresses all critical issues and several suggestions from the first design review.
...
`is_match_group_by_fields` continues to receive `&file_format` as today for single-table queries. For multi-table queries (registry has more than one entry), `SELECT *` with `GROUP BY` is an error because there is no single schema to expand into. Add a new error variant:
...
Check this condition in `parse_query` before calling `is_match_group_by_fields`: if the registry has more than one table and `Named::Star` appears in the select list alongside aggregates, return `StarGroupByMultiTable`.
